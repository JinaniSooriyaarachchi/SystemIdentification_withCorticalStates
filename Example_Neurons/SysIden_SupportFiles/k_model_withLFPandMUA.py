...
from SysIden_SupportFiles.k_layers import gaussian2dMapLayer, PowerLowNonLinearity
...
def model_pass1(Input_Shape,Filter_Size,Stride,Pool_Size,downsampImageSize, 
    ...
    lfp_driven_unit = keras.layers.Dense(1,kernel_regularizer=l2(lfp_l2_value),name="lfp_driven_unit")(lfp_data)
    
    mua_driven_unit = keras.layers.Dense(1,kernel_regularizer=l2(mua_l2_value),name="mua_driven_unit")(mua_data)
    
    additive_Layer = keras.layers.Add()([model_gaussian,lfp_driven_unit,mua_driven_unit])
    
    model_dense5 = Dense((1),weights=Initial_Dense_Values)(additive_Layer)  
    
    output = Activation('relu')(model_dense5) 
    
    model = keras.models.Model(inputs=[inputLayer,lfp_data,mua_data],outputs =output)
...
def model_pass2(Input_Shape,Filter_Size,Stride,Pool_Size,downsampImageSize,Initial_Filter_Weights,
    ...
    model2 = keras.models.Model(inputs=[inputLayer,lfp_data,mua_data],outputs =output)
